...
if df is not None:
    ...
    with st.sidebar:
        ...
        st.write("---")
        # Thiết lập mặc định phù hợp với dữ liệu độ cứng (Hardness) thực tế của bạn
        usl = st.number_input("規格上限 (USL)", value=65.0, format="%.1f")
        lsl = st.number_input("規格下限 (LSL)", value=55.0, format="%.1f")
    ...
    if len(data) > 1:
        ...
        plot_max = max(usl, ucl, max(data)) + 2
        ...
        metrics = [("樣本數 (N)", n), ("平均值 μ", f"{mean:.2f}"), ("標準差 σ", f"{std:.2f}"), 
                   ("Ca (準確度)", f"{ca:.2f}"), ("Cp (精密度)", f"{cp:.2f}"), ("Cpk (能力指數)", f"{cpk:.2f}")]
        ...
        with col_left:
            ...
            fig_hist.update_layout(
                height=300, margin=dict(l=10, r=10, t=65, b=10),
                template="plotly_white",
                title=dict(text=f"數據分佈 (Bins: {sturges_k})", font=dict(size=13), y=0.92, x=0.5, xanchor='center'),
        ...
        with col_right:
            ...
            fig_trend.update_layout(
                height=300, margin=dict(l=40, r=40, t=65, b=40),
                template="plotly_white",
                title=dict(text=f"趨勢監控 (±{sigma_val}σ)", font=dict(size=13), y=0.92, x=0.5, xanchor='center'),
