...
if df is not None:
    ...
    if len(data) > 1:
        ...
        # Mở rộng dải hiển thị để không bao giờ mất đường giới hạn
        ...
        with col_box:
            st.markdown('<div class="chart-container">', unsafe_allow_html=True)
            fig_box = go.Figure()
            # Đổi màu điểm vượt giới hạn trong Boxplot
            box_colors = ['#FF0000' if (v < lsl or v > usl) else '#0078D4' for v in data]
            fig_box.add_trace(go.Box(y=data, marker=dict(color='#0078D4', outliercolor='#FF0000'), boxpoints='all', jitter=0.3))
            
            # Giới hạn cho Boxplot
